...
CREATE INDEX IF NOT EXISTS tags__tag_repost ON tags (lower("tag"))
  WHERE lower("tag") = 'repost';
...
BEGIN
  ...
  THEN
    ...
    VALUES (NEW.id, NEW.up - NEW.down)
    ON CONFLICT (id)
      DO UPDATE SET score = NEW.up - NEW.down;
  ELSE
  ...
  THEN
    INSERT INTO controversial (item_id) VALUES (NEW.id)
    ON CONFLICT (item_id)
      DO NOTHING;
  END IF;
